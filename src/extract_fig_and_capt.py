...
def group_figures_and_captions(bboxes_fig: List, bboxes_capt: List) -> List:
    ...
    for bbox_fig in bboxes_fig:
        ...
        for bbox_capt in bboxes_capt:
            pdf_path_capt, data_type_capt, pnum_capt, caption_ind, _, _, _, _, _, _ = bbox_capt
            if pdf_path_fig == pdf_path_capt and pnum_fig == pnum_capt and figure_ind == caption_ind:
    ...
    for pdf_path in tqdm(pdf_files):
        ...
        if tree:
            ...
            for pair in pairs:
                ...
                pdf_path, data_type, pnum, ind, pwidth, pheight, left, top, right, bottom = bbox_fig
                try:
                    img = get_bb_img(pdf_path=pdf_path, page_number=pnum, left=left, top=top, right=right, bottom=bottom)
                    img_path = os.path.join(img_root, filename, "{}_{}.png".format(pnum, ind))
                    ensure_parent_dir(img_path)
                    img.save(filename=img_path)
                except ValueError:
                    continue

                pdf_path, data_type, pnum, ind, pwidth, pheight, left, top, right, bottom = bbox_capt
                text = get_bb_text(pdf_path=pdf_path, page_number=pnum, left=left, top=top, right=right, bottom=bottom)
                json_path = os.path.join(json_root, filename, "{}_{}.json".format(pnum, ind))
