...
class InferenceEvaluator:
    def __init__(
        self, ind2organ_path: str, organ2label_path: str, organ2summary_path: str, voxelman_images_path: str
    ):
    ...
    def voxels_distance(
        ...
        for i, organ_index in enumerate(organ_indices):
            ...
            if inside:
                ...
                summary_points = np.array(self.organ2summary[self.ind2organ[str(organ_index)]])
                distances[i] = np.sqrt(np.square(pred - summary_points).sum(axis=1)).min()
